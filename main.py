...
def main():
    ...
    # ── AUTO-RUN MAILER ──────────────────────────────────────────────────
    # Sends daily email digest automatically after every full run.
    # Requires GMAIL_APP_PASSWORD set as a Windows environment variable.
    # To set it: Windows search → "Edit environment variables" →
    #   User variables → New → Name: GMAIL_APP_PASSWORD  Value: your-16-char-key
    try:
        import importlib
        mailer = importlib.import_module("mailer")
        logger.info("\n📧 Sending daily email digest...")
        mailer.send_digest(new_jobs if new_jobs else export_jobs[:10])
        logger.info("✅ Email digest sent successfully")
    except ModuleNotFoundError:
        logger.warning("⚠️  mailer.py not found — skipping email digest")
    except Exception as e:
        logger.warning(f"⚠️  Email digest failed: {e}")
        logger.warning("   Check that GMAIL_APP_PASSWORD is set as a Windows environment variable")
